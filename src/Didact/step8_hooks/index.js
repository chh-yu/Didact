...
function updateFunctionComponent(fiber) {
	...
	wipFiber = fiber
	// 使用 hooks 数组以支持我们在同一个函数组件中多次调用 useState，并记录hooks序号
	hookIndex = 0
	wipFiber.hooks = []
	// 不同于普通fiber， 函数组件的fiber的子节点由函数运行得来而不是直接从 props 属性中获取
	const children = [fiber.type(fiber.props)]
	reconsileChildren(fiber, children)
}
function useState(initial) {
	// 当函数组件调用 useState，我们校验 fiber 对应的 alternate 字段下的旧 fiber 是否存在旧 hook
	// hook 的序号用以记录是该组件下的第几个 useState

	// 校验 fiber 对应的 alternate 字段下的旧 fiber 是否存在旧 hook
	// hook 的序号用以记录是该组件下的第几个 useState
	const oldHook =
	...
	const setState = (action) => {
		...
		// 将 wipRoot 设置为当前 fiber，之后我们的调度器会帮我们开始新一轮的渲染的
		wipRoot = {
			dom: currentRoot.dom,
